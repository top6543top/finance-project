...
- 비관적 락(`@Lock(LockModeType.PESSIMISTIC_WRITE)`)으로 바꾸면, "일단 시도 → 충돌 → 처음부터 재시도"하는 낙관적 재시도 폭풍 대신 DB가 요청을 대기열에 줄 세워 순서대로 처리 → 실패율은 낮아지지만, 대기 트랜잭션이 늘어날수록 응답 지연(latency)이 커지고 전체 처리량(throughput)은 오히려 떨어질 수 있음.
...
- 낙관적 락 대신 비관적 락을 쓰면 뭐가 좋아지고 뭐가 나빠지나요? → 재시도 폭풍 없이 대기열로 순서대로 처리돼 실패율은 줄지만, 대기 트랜잭션이 늘수록 응답 지연이 커지고 전체 처리량은 떨어질 수 있음. 충돌 빈도가 높은 핫스팟(계좌 1개 몰림)엔 유리, 트래픽이 여러 계좌에 분산되는 일반 상황엔 불리 — 상황에 따라 갈리는 트레이드오프.
